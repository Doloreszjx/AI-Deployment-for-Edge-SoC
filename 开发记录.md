...
# Phase 5 算子融合与执行策略优化
在 INT8 在 CPU-only 场景下无法带来收益的前提下，通过算子融合与执行策略优化，减少算子碎片化与数据重排带来的调度与内存开销。

**在端侧SoC上，真正需要考虑的不是算力不足，而是**
- 频繁Kernal Launch
- 频繁Memory read/write
- CPU和加速器之间的切换

算子融合的本质是：把多个小问题**合并**成一个硬件友好的大问题

#### 模式 1：Conv + Activation（高优先级）
现在看到的真实情况：
Conv → QuickGelu
...
- 减少 1 次 feature map 写回
- cache locality 明显改善

#### 模式 2：Conv + Reorder（非常关键）
现在看到的：
Conv (NCHW)
 → ReorderOutput
 → 下一个 Conv (NHWC)
...
#### 模式 3：Concat / Split / Reshape 链路压缩
当前形态：
Split → Conv
Split → Conv
Concat → 后续算子
...
### 为什么在CPU-Only环境下以上优化依然成立
即使在CPU-Only环境下，算子融合依然有价值，因为减少了kernel调用次数和中间tensor的内存访问。而在SoC上，这些优化会进一步放大，尤其是在CPU与NPU协同执行的场景中。

#### 1. 对 SoC / NPU 架构的建议

提供：

Conv + Activation 融合算子
...
自动识别 Conv–Activation–Reorder pattern
...
在对 YOLOv8n 的端侧 profiling 中，我发现除了 Conv 以外，**数据重排**和**大量小算子**占据了接近 40% 的推理时间。在 CPU-only 环境下，即使使用 INT8 也无法带来收益，反而放大了调度和内存访问开销。

因此我进一步从工具链角度分析了算子融合的可能性，重点包括 Conv–Activation 融合、Conv–Layout Reorder 融合以及对 Concat/Split/Reshape 等结构性算子的压缩。这类优化能够显著减少 kernel launch 次数和中间 buffer 的内存访问，在 SoC 和 NPU 场景下会带来更明显的收益。